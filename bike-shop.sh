...
RENT_MENU() {
  ...
  # if no bikes available, check if the variable is empty
  ...
  then
    ...
    # processes separately each line of text stored in the variable
    ...
    # checks if the value is not a positive integer
    ...
    then
      ...
      # if not available, check if the variable is empty
      ...
      then
        ...
        read PHONE_NUMBER
        # query a customer's name based on their phone number and save the result
        CUSTOMER_NAME=$($PSQL "SELECT name FROM customers WHERE phone = '$PHONE_NUMBER'")

        # if customer doesn't exist, check if the variable is empty
        if [[ -z $CUSTOMER_NAME ]]
        then
        ...
        # send to main menu, shows the user a clean message with customer information
...
RETURN_MENU() {
  ...
  # if not found, check if the variable is empty
  if [[ -z $CUSTOMER_ID  ]]
  then
    # send to main menu
    ...
    # if no rentals, check if the variable is empty
    ...
    # if input not rented, check if the variable is empty
    if [[ -z $RENTAL_ID ]]
    then
    # send to main menu
    MAIN_MENU "You do not have that bike rented."
    else
    # update date_returned on database
